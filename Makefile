...
CPPFLAGS:=-O2 -s ASM_JS=1 -s EXPORTED_FUNCTIONS=$(EXPORTED_FUNCTIONS) -s USE_TYPED_ARRAYS=2
EMMAKE:=emmake
...
dist/taglib.js: $(TAGLIB) src/wrapper.obj src/pre.js src/post.js src/library.js
	$(EMCXX) $(CPPFLAGS) --pre-js src/pre.js --post-js src/post.js --js-library src/library.js $(shell find build/taglib -name '*.obj') src/wrapper.obj -o $@
...
stamp-taglib-build:
	...
	mkdir -p build && \
	cd build && \
	cmake -DEMSCRIPTEN=1 -DCMAKE_TOOLCHAIN_FILE=../src/Emscripten_unix.cmake ../$(TAGLIB) && \
	$(EMMAKE) make
...
src/wrapper.obj: src/wrapper.cpp
	$(EMCXX) $(CPPFLAGS) -Ibuild/taglib/Headers -c $< -o $@
